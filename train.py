...
if __name__ == "__main__":
    ...
    # ========================================================================= #
    #   新增开关：是否启用 Axial Attention（轴向注意力）机制
    # ========================================================================= #
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = r"G:\model_code\Deeplabv3+\deeplabv3-plus.HybridASPP\model_data\deeplab_mobilenetv2.pth"
    # ---------------------------------------------------------#
    ...
    # ------------------------------------------------------------------#
    save_dir = r'G:\model_code\Deeplabv3+\deeplabv3-plus.HybridASPP\logs'
    # ------------------------------------------------------------------#
    ...
    # ------------------------------------------------------------------#
    VOCdevkit_path = r'G:\model_code\Deeplabv3+\deeplabv3-plus.HybridASPP\VOCdevkit'
    # ------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    # ------------------------------------------------------------------#
    # cls_weights     = np.array([1, 15], np.float32)
    ...
    # ========================================================= #
    # 修改：将 use_axial_attention 参数传递给 DeepLab 初始化
    # ========================================================= #
    model = DeepLab(num_classes=num_classes, backbone=backbone, downsample_factor=downsample_factor,
                    pretrained=pretrained, use_axial_attention=use_axial_attention)
    ...
    #   读取数据集对应的txt
    # ---------------------------#
    with open(os.path.join(VOCdevkit_path, r"VOC2007\ImageSets\Segmentation\train.txt"), "r") as f:
    ...
    if True:
        ...
        if local_rank == 0:
            ...
            print("\n" + "=" * 40)
            print("训练全部完成！")
            print(f"总训练耗时: {int(minutes)} 分钟 {seconds:.2f} 秒")
            print("=" * 40)
            ...
            with open(time_log_path, "a", encoding="utf-8") as f:
                f.write("=" * 30 + "\n")
                f.write(f"训练开始时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}\n")
                f.write(f"训练结束时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time))}\n")
                f.write(f"总训练耗时: {int(minutes)}m {seconds:.2f}s\n")
                f.write("=" * 30 + "\n\n")